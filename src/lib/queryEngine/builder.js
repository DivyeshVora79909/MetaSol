...
function bindWhere(filters, variables) {
  const conditions = filters.map((filter, index) => {
    if (filter.rhs === "none") return filter.definition.emit({
      lhs: filter.lhs
    });
    if (filter.rhs === "range") {
...
export function buildQuery(config, ast) {
  const variables = {
    ...
    where = bindWhere(ast.filters, variables);
  const omit = ast.options.omit.length ? `OMIT ${ast.options.omit.map(field => field.expression).join(", ")}` : "";
  const order = ast.sorts.length ? `ORDER BY ${ast.sorts.map(sort => [sort.expression, sort.collate && "COLLATE", sort.numeric && "NUMERIC", sort.order].filter(Boolean).join(" ")).join(", ")}` : "";
  const fetch = ast.fetch.length ? `FETCH ${ast.fetch.map(field => field.expression).join(", ")}` : "";
  const timeout = ast.options.timeout ? `TIMEOUT ${ast.options.timeout}` : "",
    parallel = ast.options.parallel ? "PARALLEL" : "",
    explain = ast.options.explain ? `EXPLAIN${ast.options.explain === "FULL" ? " FULL" : ""}` : "";
